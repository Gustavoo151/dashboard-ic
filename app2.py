...
df_cidade = pd.read_csv('Data/RELATORIO_DTB_BRASIL_MUNICIPIO.csv')
...
def exemplos_subgrupo_bar(selected_id):
    ...
    figura_Tp_Fp.update_xaxes(tickvals=[0, 1], ticktext=[f'Com baixo peso', 'Sem baixo peso'])
...
def update_percentage_text(selected_id):
    ...
    return [html.Span(f'A ', className='text-normal'),
            html.Span(f'cidade de nome_cidade teve', className='text-highlight'),
            html.Br(),
